...
class TextBookBloc extends Bloc<TextBookEvent, TextBookState> {
  ...
  Future<void> _onLoadContent(
    ...
    try {
      final contentList = await _repository.getBookContentList(book.id);
      final links = await _repository.getBookLinks(book.id);
      final tableOfContents = await _repository.getTableOfContents(book.id);
      final availableCommentators =
          await _repository.getAvailableCommentators(book.id);
      ...
      emit(TextBookLoaded(
        ...
        activeCommentators: initial.commentators,
  ...
  void _onUpdateFontSize(UpdateFontSize event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(fontSize: event.fontSize, selectedIndex: currentState.selectedIndex));
    }
  }
  void _onToggleLeftPane(ToggleLeftPane event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(showLeftPane: event.show, selectedIndex: currentState.selectedIndex));
    }
  }
  void _onToggleSplitView(ToggleSplitView event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(showSplitView: event.show, selectedIndex: currentState.selectedIndex));
    }
  }
  void _onUpdateCommentators(UpdateCommentators event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(activeCommentators: event.commentators, selectedIndex: currentState.selectedIndex));
    }
  }
  void _onToggleNikud(ToggleNikud event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(removeNikud: event.remove, selectedIndex: currentState.selectedIndex));
    }
  }
  void _onUpdateVisibleIndecies(UpdateVisibleIndecies event, Emitter<TextBookState> emit) async {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      String? newTitle;
      if (event.visibleIndecies.isNotEmpty) {
        newTitle = await refFromIndex(event.visibleIndecies.first, Future.value(currentState.tableOfContents));
      }
      int? index = currentState.selectedIndex;
      if (!event.visibleIndecies.contains(index)) {
        index = null;
      }
      emit(currentState.copyWith(visibleIndices: event.visibleIndecies, currentTitle: newTitle, selectedIndex: index));
    }
  }
  void _onUpdateSelectedIndex(UpdateSelectedIndex event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(selectedIndex: event.index));
    }
  }
  void _onTogglePinLeftPane(TogglePinLeftPane event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(pinLeftPane: event.pin, selectedIndex: currentState.selectedIndex));
    }
  }
  void _onUpdateSearchText(UpdateSearchText event, Emitter<TextBookState> emit) {
    if (state is TextBookLoaded) {
      final currentState = state as TextBookLoaded;
      emit(currentState.copyWith(searchText: event.text, selectedIndex: currentState.selectedIndex));
    }
  }
}
